...
from bintensors.tensorflow import load, load_file, save, save_file, safe_open, save_with_checksum


def _compare_tf_array(lhs, rhs) -> bool:
    return tf.reduce_all(tf.equal(lhs, rhs))
...
def create_gpt2_numpy_dict(n_layers: int) -> Dict[str, tf.Tensor]:
    tensors = {}
...
def test_save_and_load_gpt2_tensors_dict_tf():
    small_gpt2 = create_gpt2_numpy_dict(2)
    buffer = save(small_gpt2)
    model = load(buffer)
    assert all((_compare_tf_array(small_gpt2[key], model[key]) for key in small_gpt2.keys()))


def test_save_and_load_zero_sized_tensors_tf():
    _SHAPES_A, _SHAPES_B = (0,), (0, 0, 0)
    tensor_dict = {"ln.weight": tf.zeros(_SHAPES_A), "ln.bias": tf.zeros(_SHAPES_B)}
...
def test_tensor_dtype_roundtrip_tf():
    tensor_dict = {
        "float32": tf.zeros((10, 10), dtype=tf.float32),
        "float64": tf.zeros((10, 10), dtype=tf.float32),
        "int32": tf.zeros((10, 10), dtype=tf.int32),
        "int64": tf.zeros((10, 10), dtype=tf.float32),
    }
    buffer = save(tensor_dict)
    loaded_dict = load(buffer)

    for key, value in tensor_dict.items():
        assert _compare_tf_array(loaded_dict[key], value)
        assert loaded_dict[key].dtype == value.dtype


def test_save_and_load_large_gpt2_dict_tf():
    tensor_dict = create_gpt2_numpy_dict(20)
    buffer = save(tensor_dict)
    loaded_dict = load(buffer)
    assert all((_compare_tf_array(loaded_dict[key], value) for key, value in tensor_dict.items()))


def test_tensor_dict_with_mixed_shapes_tf():
    tensor_dict = {
        "scalar": tf.constant([5.0]),
        "vector": tf.constant([1.0, 2.0, 3.0]),
        "matrix": tf.constant([[1.0, 2.0], [3.0, 4.0]]),
        "tensor3d": tf.random.normal((2, 3, 4)),
    }
...
def test_invalid_tensor_dict_raises_error_tf():
    invalid_dict = {"valid": tf.zeros((5, 5)), "invalid": "string_value"}

    with pytest.raises(Exception):
        _ = save(invalid_dict)


def test_save_file_and_load_file_consistency_tf():
...
def test_safe_open_access_and_metadata_tf():
    tensor_dict = create_gpt2_numpy_dict(1)
    with tempfile.NamedTemporaryFile() as tmp:
        filename = tmp.name
...
def test_checksum_two_diffrent_models_tf():
    model_1 = {"ln.weight": tf.random.normal((10, 10)), "ln.bias": tf.random.normal((10,))}
    model_2 = {"ln.weight": tf.random.normal((10, 10)), "ln.bias": tf.random.normal((10,))}
...
def test_checksum_two_same_models_tf():
    model_1 = {"ln.weight": tf.zeros((2, 2)), "ln.bias": tf.zeros((10))}
    model_2 = {"ln.weight": tf.zeros((2, 2)), "ln.bias": tf.zeros((10))}
